...
namespace MappingTools
{
    class TemplatesTransFuncs
    {
        ...
        public static void SendStrToOutlookForIntertrade(string sendTo, string subject, string mailBody,string link,string linkDetail)
        {
            Outlook.Application olApp = new Outlook.Application();
            Outlook.MailItem mailItem = (Outlook.MailItem)olApp.CreateItem(Outlook.OlItemType.olMailItem);
            mailItem.To = sendTo;
            ...
            mailItem.BodyFormat = Outlook.OlBodyFormat.olFormatHTML;
            string content = "";
            content += ConvertToHTMLFormatForIntertrade(RemoveEmailAndSubject(mailBody),link,linkDetail);
            mailItem.HTMLBody = content;
            mailItem.Display();
            mailItem = null;
            olApp = null;
        ...
        public static string ConvertToHTMLFormatForIntertrade(string str,string link,string linkDetail)
        {
            ...
            StreamReader sr = new StreamReader(@"VANTemplateEmialFormat.html");
            string htmlToStr = sr.ReadToEnd();
            htmlToStr = htmlToStr.Replace("<p></p>", formatedStr).Replace("<p class=\"MsoNormal\">" + linkDetail + "<o:p></o:p></p>","<p class=\"MsoNormal\"><a href=\""+link+"\">"+linkDetail+"</a></p>");
            return htmlToStr;
        }
